...
export interface DbStoreIndex<T> {
  since: number;
  keyPath: string | string[];
  options?: IDBIndexParameters;
}

export interface DbStoreOptions<T> {
  /**
   * Defaults to false
   */
...
export interface DbStore<T> {
  ...
  indexes?: Record<string, DbStoreIndex<T>>;
  options?: DbStoreOptions<T>;
}
...
const haveStoresSameProps = <T>(
  ...
  const keyPath = store.options?.keyPath || null; // Default = null
  return (
...
const haveIndexesSameProps = <T>(
  index: DbStoreIndex<T>,
  indexDB: IDBIndex,
): boolean => {
...
export interface LgIndex<
  T,
  O extends DatabaseSchema['stores'][string]['indexes'][string],
> extends IDBIndex {
...
export interface LgObjectStore<T, O extends DatabaseSchema['stores'][string]>
  ...
  index<K extends keyof O['indexes']>(name: K): LgIndex<T, O['indexes'][K]>;
}
