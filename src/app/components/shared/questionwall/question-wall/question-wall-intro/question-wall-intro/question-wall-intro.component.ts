...
export class QuestionWallIntroComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    setTimeout(() => {
      const background = this.ref.nativeElement.getElementsByClassName('background')[0];
  ...
  private particleAnimation() {
    ...
    const particleList: HTMLElement[] = Array.from(container.getElementsByClassName('particle-element'));
    particleList.forEach(e => {
      e.style.transform = 'translate(' + toCompX(0) + ',' + toCompY(0) + ')';
    });
    setTimeout(() => {
      particleList.forEach(e => {
        e.style.transition = 'transform 0.9s ease, opacity ' + (Math.random() * 0.9) + 's ease';
      });
    }, 0);
    const random = () => (Math.random() * 2) - 1;
    setTimeout(() => {
      particleList.forEach(e => {
        e.style.transform = 'translate(' + toCompX(random()) + ',' + toCompY(random()) + ') rotate(' + (Math.random() * 50) + 'deg)';
        e.style.width = 10 + 'px';
