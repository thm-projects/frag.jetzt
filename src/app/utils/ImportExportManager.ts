...
export class ImportExportManager {
  ...
  static createQuillMapper<T>(
    ...
    return {
      ...
      valueMapper: {
        ...
        import: (cfg, val: string, c?: Partial<T>) =>
          fill(
  ...
  static deserializeQuill(value: string): string {
    ...
    while ((m = regex.exec(value)) !== null) {
      ...
      if (num) {
        const [attr, attrStr] = this.deserializeQuillAttributes(
          num,
          last,
          m[0],
        );
      ...
      const contentPayload = unescapeHard(
        m[0].substring(last + 1, m[0].length - 1),
      );
      switch (content) {
        case 'i':
      ...
      if (typeof e === 'string') {
        ...
        if (e.attributes) {
          return (
            acc +
            '{' +
            this.serializeQuillAttributes(e.attributes) +
            hardVerify(e['insert']) +
            '}'
          );
        }
  ...
  private static deserializeQuillAttributes(
    count: number,
    last: number,
    objStr: string,
  ): [unknown, string] {
