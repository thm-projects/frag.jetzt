...
const lang = {
  de: {
    runRoot: 'Dieses Skript sollte als User ausgeführt werden, da eventuell sonst nicht auf die Dependencies zugegriffen werden kann. Wenn du standardmäßig alles als Root-User machst, fahre fort.\n\nMöchtest du fortfahren? (Ja [y/j/o] / Nein [n]): ',
    ...
    sureDelete: 'Bist du sicher, dass du alle Daten löschen möchtest? (Ja [y/j/o] / Nein [n]): '
  },
  en: {
    runRoot: 'This script should be run as a user, as otherwise access to the dependencies may not be possible. If you do everything as a root user by default, continue.\n\nDo you want to continue? (Yes [y/j/o] / No [n]): ',
    ...
    sureDelete: 'Are you sure you want to delete all data? (Yes [y/j/o] / No [n]): '
  },
  fr: {
    runRoot: 'Ce script doit être exécuté en tant qu\'utilisateur, sinon l\'accès aux dépendances peut ne pas être possible. Si vous faites tout en tant qu\'utilisateur root par défaut, continuez.\n\nVoulez-vous continuer? (Oui [y/j/o] / Non [n]): ',
    ...
    sureDelete: 'Es-tu sûr de vouloir supprimer toutes les données? (Oui [o] / Non [n]): '
  }
};
...
async function checkProgram(program) {
  // unix || windows
  return (await checkCmd('which', program)) || (await checkCmd('where', program));
}

async function getLanguage(langObj) {
  let langKey;
  try {
    langKey = readFileSync(__dirname + "/lang.txt", "utf-8").toString().trim();
  } catch (e) {
...
if (!existsSync(newDir + '/frag.jetzt-docker-orchestration')) {
  ...
  await run('git', ['clone', 'git@git.thm.de:arsnova/frag.jetzt-docker-orchestration.git']);
...
while (true) {
  ...
  if (option === '1') {
    ...
    await run('./setup.sh', ['--recreate-env', '--recreate-config', '--no-frontend', '--sample-data'], execOptions);
