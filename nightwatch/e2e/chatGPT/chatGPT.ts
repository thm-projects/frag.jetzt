import { defineStep as And, Then, When, Given } from '@cucumber/cucumber';

Given('I am on the overview of a prepared room', async () => {
  await browser.navigateTo('/user');
  await browser.waitForElementVisible('#session_id-input');
  await browser.setValue('#session_id-input', process.env['TEST_ROOM_SHORTID']);
  await browser.click('#session_enter-button');
  await browser.waitForElementNotPresent('#session_id-input');
  await browser.expect
    .url()
    .to.startWith(
      '/participant/room/' + process.env['TEST_ROOM_SHORTID'] + '/comments',
    );
});
...
When('I open the chatGPT assistent', () => {
  ...
  cy.url().should(
    'include',
    '/participant/room/' + Cypress.env('preparedRoomId') + '/gpt-chat-room',
  );
...
Then('I ask the assistent a prepared prompt', () => {
  ...
  cy.get('mat-form-field')
    ...
    .type('Linux-Terminal');
  cy.wait(500);
  cy.get('mat-option.ng-star-inserted[ng-reflect-value="Linux-Terminal"]')
    .last()
    .click();
...
And('The assistent should answer the prompt', () => {
  ...
  cy.wait('@streamInterrupt', { timeout: 100000 }).then((res) => {
    cy.get('div.gpt.ng-star-inserted')
      .find('p')
      .then(($el) => {
...
And(
  'I should be able to assume the answer to the QnA forum of the room',
  () => {
    ...
    cy.get('app-gptrating-dialog')
      .find('button.primary-confirm-button')
      .click();
    cy.wait(500);
    cy.url().should(
      'include',
      '/participant/room/' + Cypress.env('preparedRoomId') + '/comment',
    );
  },
);

When('I am asking the question {string} on the forum', (question: string) => {
  cy.wait(1000);
  ...
  cy.url().should(
    'include',
    '/participant/room/' + Cypress.env('preparedRoomId') + '/comments',
  );
...
Then('ChatGPT can let reply on it', () => {
  ...
  cy.get('mat-select[ng-reflect-panel-class="gpt-chat-room-select"]')
    .first()
    .click();
  cy.wait(500);
